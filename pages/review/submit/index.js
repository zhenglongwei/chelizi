...
Page({
  ...
  updateProgress() {
    ...
    if (module3.completion_urls && module3.completion_urls.length >= 2) {
      done++;
    } else if (module3.completion_images && module3.completion_images.length >= 2) {
      done++;
    }
    ...
    if (m2Required) {
      ...
      const matCount = Math.max(module2.materialUrls?.length || 0, module2.materials?.length || 0);
  ...
  onM2ChooseMaterial() {
    ...
    const remain = max - (module2.materials?.length || 0);
    ...
    wx.chooseMedia({
      ...
      success: (res) => {
        ...
        this.setData({ 'module2.materials': materials }, () => this.updateProgress());
      }
    });
  },
  ...
  onM2DelMaterial(e) {
    ...
    module2.materials.splice(idx, 1);
    module2.materialUrls.splice(idx, 1);
  ...
  onM3ChooseCompletion() {
    ...
    const remain = 8 - (module3.completion_images?.length || 0);
    ...
    wx.chooseMedia({
      ...
      success: (res) => {
        ...
        this.setData({ 'module3.completion_images': completion_images }, () => this.updateProgress());
      }
    });
  },
  ...
  onM3DelCompletion(e) {
    ...
    module3.completion_images.splice(idx, 1);
    module3.completion_urls.splice(idx, 1);
  ...
  async onSubmit() {
    ...
    try {
      ...
      for (let i = 0; i < (module3.completion_images?.length || 0); i++) {
        if (!completionUrls[i]) {
          completionUrls[i] = await uploadImage(module3.completion_images[i]);
        }
      }
      ...
      if (orderTier >= 3) {
        for (let i = 0; i < (module2.materials?.length || 0); i++) {
          if (!materialUrls[i]) {
            materialUrls[i] = await uploadImage(module2.materials[i]);
          }
        }
