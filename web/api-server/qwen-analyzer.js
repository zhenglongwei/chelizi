...
- **repair_suggestions**：至少 2 条，且均须与**用户原话可逐条对应**或与本条 damages 一致；**仅当用户文字明确写涉水/泡水/进水**时才允许水损拆检类工艺；禁止「全车电路预防性检测」等与用户描述无关的凑数项。
- **最低产出**：第一辆车 **damages 至少 2 条**，且**两条均须与用户文字中的不同现象或同一现象的不同侧面直接相关**；禁止用一条写用户说的、另一条写用户完全没说的来凑数。
