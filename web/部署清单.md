...
```bash
cd c:\Users\longwei\WeChatProjects\zhejian\web
npm run build
```
...
>
> ```bash
> mysql -u root -p zhejian < database/migration-20260226-material-audit.sql
> mysql -u root -p zhejian < database/migration-20260226-merchant-evidence.sql
> mysql -u root -p zhejian < database/migration-20260226-q3-fault-evidence.sql
> ```
>
> **奖励金规则 JSON（基础轨简化后）**：若需将线上 `reward_rules` 中旧版封顶/浮动字段一次性清零，在已上传代码且 `api-server` 已 `npm install` 的前提下，于 **`/var/www/simplewin` 的上级或含 `web/` 的仓库根**执行：`node web/scripts/normalize-reward-rules-db.js --dry-run` 核对后去掉 `--dry-run`。详见 `web/deploy.md`。
...
crontab -e
# 添加：*/2 * * * * /var/www/simplewin/scripts/cron-send-delayed-bidding-messages.sh >> /var/log/zhejian-cron.log 2>&1
```
...
# 3. 使用 pm2
cd /var/www/simplewin/api-server && pm2 start server.js --name zhejian-api
```
...
**现象**：浏览器访问 [https://simplewin.cn](https://simplewin.cn) 时，控制台或网络面板显示 `index-xxx.js` 或 `index-xxx.css` 返回 404。
