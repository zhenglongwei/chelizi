...
• AI诊断助手（模块化）：支持图片诊断、故障码诊断、症状文本诊断，统一输出“损失报告（AI）”模板（明显损伤/可能损伤/维修建议），可在小程序、官网、公众号H5、独立H5中复用；当前以页面能力对外，API能力暂不商业开放
• 配件验真聚合（模块化）：已完成模块化预研与回填留痕机制；因缺少稳定公开官方验真渠道，当前暂缓对外上线，不作为正式用户入口
...
平台采用“佣金+返点”的商业模式：
1.3.1 佣金收取
向维修厂收取维修金额的佣金（比例按订单分级、复杂度等由 `reward_rules` 与订单字段 `commission_rate` 确定，文档中历史表述「12%」为基准参考）。佣金在**车主确认完工**后进入平台计费流程，采用**两阶段**：
- **阶段 A（完工确认）**：以 `actual_amount`（若有）否则 `quoted_amount` 为基数计算**暂计佣金** `commission_provisional`，并执行收款：支持**预充值账户自动扣款**或**逐单微信支付**（服务商在小程序内支付至平台微信商户号）。
- **阶段 B（凭证 finalize）**：车主或门店补充**维修款支付凭证**并录入最终 `actual_amount` 后，重算**最终佣金** `commission_final`，与阶段已收金额的差额多退少补（账户内虚拟余额调整 + 必要时再次调起微信支付或原路退款流程）。
**保险事故车**：若配置 `commission_waive_insurance=1`（系统配置），则该单不向门店收取佣金（记 `commission_status=waived_insurance`）；否则与自费单同等收取。
**预充值账户**：服务商可充值至佣金账户，订单发生时自动扣款；余额可申请**原路退款**（需满足可用余额大于冻结额度等规则，详见服务端与协议）。账户需展示**收支明细**；**余额不足**时通过订阅消息等提醒并及时补缴。
1.3.2 返点分配
用户可获得最高10%的返点，分阶段发放：
评价类型	返点比例	获取条件
首次评价	8%	维修完成后7天内提交评价
...
1.3.4 代理人（推荐）分佣（与评价激励并行）
- **绑定**：车主通过分享落地参数绑定一级推荐人（`users.referrer_user_id`，仅首次）；绑定后该用户标记为**分销客源买家**（`is_distribution_buyer=1`）。**分享入口**：**整页分享**（首页、口碑、店铺详情、我的等）使用微信右上角「···」转发；自定义导航页在 `onShow` 中调用 `wx.showShareMenu`（封装见 `utils/show-share-menu.js`），并实现 `onShareAppMessage`，路径统一带 `ref`；**单条评价**在口碑流、店铺详情评价区提供「分享评价」按钮（带 `review_id` 等落地）。与 `app.js` 写 `pending_referrer_user_id`、登录后 `bindReferrer` 一致。
- **推荐佣金**：自订单实收佣金 **C** 计提（与 `computeOrderCommissionAmount` 一致），**一级 10%·C**、**二级 2%·C**（二级为一级推荐人的推荐人）；在月度结算任务中入账 `referral_l1_bonus` / `referral_l2_bonus`，**不计入**单笔订单 **80%·C** 车主侧激励硬帽（与 `rebate`/`like_bonus`/`conversion_bonus` 等分列）。
- **与评价激励**：分销客源买家对他人评价的**互动轨**（如 `like_bonus`）**不**减半；**内容转化**与**事后验证补发**对评价作者的金额**减半**（归因上成交侧重代理人）。**无该单下单前浏览**则作者**无**转化类收入（与既有 `conversion-bonus-service` / `settlePostVerifyBonus` 浏览校验一致）。
- **法务与税务**：二级长期分佣、对外展示文案、劳务税/提现合规须由**法务与财务**书面确认后再全量开放；本节为产品与技术口径摘要。奖励金在系统内按**居民劳务报酬**预扣（见 `labor-remuneration-withhold.js`），**按笔在入账/余额增加时**完成代扣，**提现**不再就余额重复预扣；是否须改为按支付日或按月合并，以财务书面为准。

1.4 技术架构
平台基于阿里云ECS + MySQL + OSS构建，提供稳定可靠的云服务。
1.4.1 技术栈
• 前端：微信小程序原生框架
• 外部触点：官网H5 / 公众号网页 / 独立H5（通过 OpenAPI 调用能力模块）
...
备注	1. 竞价有效期2小时（截止后已报价仍有效，用户可随时选择） 2. 至少邀请5家维修厂 3. 用户可随时结束竞价
2.2.4 接口说明
方法	接口	说明	参数
...
备注	1. 返点发放到用户余额 2. 余额大于0即可申请提现 3. 返点不可叠加超过10%
2.5.4 接口说明
方法	接口	说明	参数
...
功能ID	F-USER-001
功能描述	用户申请提现返点余额
操作流程	1. 用户点击提现按钮 2. 输入提现金额（大于0即可） 3. 确认提现 4. 调用 POST /api/v1/user/withdraw；若已配置商家转账到零钱，服务端创建微信转账单并返回 package_info 5. 小程序调 wx.requestMerchantTransfer 拉起微信「确认收款」页 6. 用户确认后资金入账微信零钱；终态以微信回调为准
预期结果	1. 显示可提现余额 2. 确认后进入微信官方收款确认流程（与商户平台展示一致）3. 成功后在交易明细中记一笔提现；失败或撤销时退回小程序内余额
备注	1. 单日累计最高5000元（产品规则）2. 未配置商家转账/回调域名时可为 legacy：仅记账待人工处理（见部署文档）3. 回调 /api/v1/pay/wechat/reward-transfer-notify；极小金额以微信侧规则为准
2.6.4 接口说明
方法	接口	说明	参数
...
POST	/api/v1/user/withdraw	申请提现	{amount, real_name?}（微信通道且单笔≥2000元时必填；可选填用于更新 users.withdraw_real_name 预填）	{withdraw_id, transfer_mode, action?, package_info?, amount?, warning?, hint?…}	400-校验失败, 401-未登录, 409-legacy 待处理冲突
POST	/api/v1/user/withdraw/reconcile	同步提现单据	{withdraw_id?}	{withdrawal, package_info?, can_request_transfer}	401
POST	/api/v1/user/withdraw/cancel-pending	撤销待确认转账	{withdraw_id?}	{ok, withdrawal, …}	400/401
...
| /api/v1/merchant/quote | POST | 提交预报价（含分项、拆解检测费披露：`disassembly_fee` / `disassembly_fee_waived` / `disassembly_fee_note`） |
