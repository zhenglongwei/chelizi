...
.cursor/rules 无具体扣分，采用 [《评价内容设置规范》（已归档）](./已归档/评价内容设置规范.md)：
...
**材料审核判定原则**（2026-03 优化）：
- **通过**：结算单有效 + **车辆一致性**（照片中车辆与订单一致）+ 施工图覆盖维修部位且车辆外观正常 + 物料照片相关。
- **不通过**：结算单无效；或**车辆不一致**（如上传其他车辆照片，车牌/品牌/车型/颜色与订单不符）；或车辆仍有明显损伤（凹陷、破损、未喷漆等）；或照片与订单车型/部位明显不符。
- **车辆一致性**：若订单有车牌号，照片中车牌需与订单一致；若车牌不可见，可结合品牌、车型、颜色综合判断。防止服务商上传另一辆车的完整照片冒充本单。
- **重要**：若车辆外观完好、无明显损伤，应视为维修已完成，不得以「未见修复痕迹」为由不通过。专业维修后外观恢复如新属正常情况。
...
| **订阅消息** | 已实现 | 通过 `subscribe-message-service` 发送小程序订阅消息，需服务商绑定 openid 并授权 |
...
1. 执行迁移：`mysql -u root -p zhejian < web/database/migration-20260226-material-audit.sql`  
   `mysql -u root -p zhejian < web/database/migration-20260226-merchant-evidence.sql`  
   `mysql -u root -p zhejian < web/database/migration-20260226-q3-fault-evidence.sql`  
   `mysql -u root -p zhejian < web/database/migration-20260226-appeal-status4.sql`
2. 上传代码并重启 API 服务
