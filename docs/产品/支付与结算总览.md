...
**时点区分（避免混淆）**：竞价侧 **商家接单即视为成交，用户即可预约到店**；**不要求**在接单时按报价付清维修款。

**双阶段报价与锁价（产品约定，与《维修方案调整与确认流程》一致）**：

- **预报价**：竞价阶段服务商依据照片/描述提交的报价，写入 `quotes`；用户选厂生成订单时，将预报价**快照**记入订单（如 `pre_quote_snapshot`），用于评价展示与「预↔终」偏差统计。
- **最终报价**：到店检测后形成；**须车主在小程序确认**后写入**最终报价快照**（如 `final_quote_snapshot`）并进入**锁价**状态。**向车主应收金额以已确认的最终报价为准**，不得再新增收费项目或费用；施工增项成本由服务商自行承担。
- **结算单**：仅作履约与对账凭证；**不得突破**已确认最终报价向车主加收。自费经平台支付时，应付金额与分账基数与最终报价快照一致（与历史表述「完工后以结算单为准」统一为：**结算单金额须与锁价一致**，若线下另有约定须不在平台内形成加价链路）。
- **商户提交结算金额**：`POST /api/v1/merchant/orders/:id/commission-finalize` 写入的 `actual_amount`，在订单已锁价（`final_quote_status=2`）时**不得高于**车主已确认的最终报价金额（实现见 `commission-wallet-service.finalizeCommissionProof`），与上述口径一致。
- **维修价款支付时点**：仍在 **服务完工** 后按上述锁价金额支付或对账；竞价列表中的金额为**预报价**，页面须向车主说明两阶段差异。保险理赔款仍 **保司 → 店**，平台佣金路径见第四节模式二。

### 产品线矩阵

| 产品线 | 代表数据 / 入口 | 与支付关系（当前实现） |
|--------|-----------------|------------------------|
| **标品直购** | `product_orders`；`product-order-service.js`（`createPrepay` 等） | 车主在小程序内对**订单全额**走微信 JSAPI；回调入账后，平台按 `settings.product_order_platform_fee_rate` 抽成（**后台入口**：`/admin/commission-rules` 佣金规则配置；实现见 `shop-income-service.js`），余款记入店铺 **`income_balance`**（货款账本），可走微信零钱/对公提现（已实现）。 |
| **竞价维修** | `biddings` → `orders`；`bidding-service.js`（如 `selectQuote`） | **产品约定**：**商家接单后订单即视为成交**，此时用户即可发起预约。竞价阶段为**预报价**；**最终报价经车主确认后锁价**，**维修价款以锁价为准**（结算单须一致）。**资金流**：**自费**车主 → 平台 → 服务商，**佣金在支付分账中一次完成**（第四节模式一）；**保险**保司 → 服务商，平台**不触达理赔款**，**佣金按报价预扣、按结算多退少补**（第四节模式二）；佣金暂计/轧差所依据的金额口径与锁价后的 `actual_amount` 对齐。**当前实现（仓库）**：字段与锁价状态机见《数据库设计文档》订单扩展；**自费**在商户提交结算后，车主可 `POST /api/v1/user/orders/:id/repair-prepay` 微信 JSAPI 支付；**仅保险单**（`is_insurance_accident`）完工走 `commission-wallet-service` 阶段 A/B。 |
...
- **支付时点（维修价款）**：**自费与保险均在服务完成后**，按**车主已确认的最终报价（锁价）**对应金额收款或对账；竞价 `quoted_amount` 为**预报价**；订单内 `pre_quote_snapshot` / `final_quote_snapshot` 为权威展示与统计口径。  
...
**佣金基准比例与业务细则以《03-全链路激励驱动体系》为准；运营后台在「佣金规则配置」（`/admin/commission-rules`）维护维修/标品费率，在「奖励金规则配置」维护奖励金相关参数。本文描述「钱怎么收」及与付款方相关的实现差异。**
...
### 4.5 自费 vs 保险：维修佣金（固定产品线）

- **维修单**：`orders.commission_rate` 由 **`commissionRepair`** 决定：先按 `is_insurance_accident` 取 **自费 / 保险** 分支，再按报价行解析出的**类目**查 `byCategory`，无则使用该分支 **default（%）**。无订单金额分档、无合规率对佣金率的乘数、无「价格竞争力」乘数。  
- **标品**：仍走 `product_order_platform_fee_rate`，与 `commissionRepair` 分轨。  
- **奖励金 70% 红线**：按该单**实收佣金**计算；与费率同升降。
...
| 官网历史成交参考价（公开） | `web/api-server/services/historical-fair-price-service.js`，`GET /api/v1/public/historical-fair-price` |
...
1. 订单**完工**且服务商/用户侧已确认**结算金额**（**须与车主已确认的最终报价/锁价一致**；可与阶段 B 提交 `actual_amount` 同一节点或紧接其后），车主看到应付维修款，发起支付。  
...
## 附录 C：维修固定佣金与竞价排序参数（摘要）

### C.1 `commissionRepair`（reward_rules，模块 5）

| 字段 | 说明 |
|------|------|
| `self_pay.default` | 车主自费默认佣金（%），配置缺失时服务端默认 6 |
| `self_pay.byCategory` | 类目 → 费率（%），键与 `repair-commission-category` 解析结果一致 |
| `insurance.default` | 保险事故车默认佣金（%），配置缺失时服务端默认 12 |
| `insurance.byCategory` | 同上，保险分支 |

标品费率：`settings.product_order_platform_fee_rate`，后台在 **佣金规则配置** 页维护。

### C.2 竞价侧（settings.biddingDistribution，默认）

| 参数 | 默认值 | 含义 |
|------|--------|------|
| quoteSortWeightMatchInsurance / ReasonInsurance | 0.7 / 0.3 | 保险：综合排序更偏匹配分 |
| quoteSortWeightMatchSelfPay / ReasonSelfPay | 0.55 / 0.45 | 自费：提高报价合理性权重 |
| matchDeviationWeightInsurance | 0.88 | 匹配分内报价历史偏差项略降 |
| matchDeviationWeightSelfPay | 1.12 | 自费略放大偏差项影响 |

报价合理性排序仍可用定损 `total_estimate` 等与《06》公允思路一致的数据源，**与维修佣金率脱钩**。
