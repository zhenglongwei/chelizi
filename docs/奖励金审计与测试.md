...
| 场景 | 执行位置 | 命令 |
|------|----------|------|
| **本地开发** | 项目根目录 `c:\...\chelizi` | `REWARD_AUDIT_LOG=1 node web/api-server/server.js` |
| **生产（pm2）** | 服务器 | 在 pm2 环境或 `.env` 中加 `REWARD_AUDIT_LOG=1`，重启后生效 |
| **查看日志** | 服务器 | `grep "\[REWARD-AUDIT\]" ~/.pm2/logs/chelizi-api-out-*.log` |

**说明**：审计日志输出到 API 进程的 stdout，与普通日志混在一起。pm2 默认写入 `~/.pm2/logs/`，可按需重定向到自定义文件。

**pm2 启用审计日志**：在 api-server 的 `.env` 中增加 `REWARD_AUDIT_LOG=1`，或 pm2 ecosystem 的 `env` 中配置，然后 `pm2 restart chelizi-api`。
...
**执行位置**：项目根目录（如 `c:\Users\longwei\WeChatProjects\chelizi` 或服务器上的 `/var/www/simplewin`）
