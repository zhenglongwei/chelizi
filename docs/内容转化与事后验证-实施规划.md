...
- **内容转化奖金池（与事后验证共帽）** = 该笔订单实收佣金 × **10%**（默认，见 `reward_rules.platformIncentiveV1.conversionPoolShare`；与事后验证从同一帽内互斥分配）
...
| 内容停留 | 30%  | ≥3 分钟 3.0；1–3 分钟 2.0；≥10 秒起最低档 1.0（与有效赞阅读门槛一致）           | `review_reading_sessions` 汇总                             |
| 内容匹配 | 20%  | 同车型+同项目 3.0；同车型+同类 2.0；同价位+同类 1.0                        | `biddings.vehicle_info`、`quotes.items` vs `reviews` 对应订单 |
| 内容价值 | 10%  | 平台化默认 **1.0（中性化）**，与 `content_quality_level` 脱钩；可配置恢复旧乘子 | `reviews.content_quality_level` + `reward_rules`         |
...
| **阶段 4** | 内容价值权重：默认中性化 1.0（可选映射）                                      | 0.5 天 |
...
- 同一笔订单的奖金池（佣金 **10%** 共帽）：若已分配内容转化，则事后验证不补发；若未分配，则事后验证可在帽内补发（与 `settlement-service` / `conversionPoolShare` 一致）
- **跨周期扣减**：点赞先于订单产生时，该点赞可能已按常规点赞结算；内容转化发放时扣除已发常规点赞金额，实发 = 转化应得 − 已发
