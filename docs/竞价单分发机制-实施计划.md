...
| **距离** | | |
| （已移除兜底扩大） | — | 严格按用户选择距离，不自动扩大；无服务商时仅提醒用户扩大范围 |
| **梯队** | | |
...
| L3 | 二类及以上 | 三类不可；二类若校验技师持证，宜满足**任一套体系**的相应等级（如检测维修维修士或汽车维修工中级工及以上；以线上 `checkShopQualificationForBidding` 实现为准） |
| L4 | 一类 | 二类/三类不可；一类宜具备**任一套体系**的高等级（如检测维修工程师或汽车维修工技师及以上）+ 设备（可首期放宽为仅一类；以线上实现为准） |
...
| 订阅消息 | 已实现：创建 `merchant_messages` 时同步调用 `subscribe-message-service` 发送小程序订阅消息 |
