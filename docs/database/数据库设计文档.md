...
为事故车维修平台（车厘子）提供完整的数据存储方案，支撑 AI 定损、竞价报价、订单、评价、奖励金等核心业务。评价与激励体系以《评价和激励体系.md》为准。
...
users ──┬── damage_reports
        ...
        │       │
        │       └── orders ──── reviews ──── review_dimensions
        │                         │              review_audit_logs
        ├── transactions         └── complexity_upgrade_requests (order_id)
        ├── withdrawals
        ├── user_messages
...
| level | TINYINT UNSIGNED | DEFAULT 1 | 用户等级 1–8 |
...
| completion_images | JSON | | 完工实拍图 URL 数组【评价体系新增】 |
...
| user_id | VARCHAR(32) | NOT NULL, FK→users | 用户 ID |
| type | VARCHAR(20) | NOT NULL | rebate/withdraw/recharge 等 |
...
| 奖励金规则 | 按《评价和激励体系.md》三级校准（复杂度+车价+维修金额），来自 repair_complexity_levels、reward_rules |
