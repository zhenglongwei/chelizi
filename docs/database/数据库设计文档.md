...
| commission_status | VARCHAR(32) | | 佣金收款：waived_insurance / legacy_exempt / paid_provisional / arrears / awaiting_pay / finalized |
| commission_provisional | DECIMAL(10,2) | | 阶段 A 暂计佣金（车主确认完工时） |
| commission_final | DECIMAL(10,2) | | 阶段 B 最终佣金（凭证 finalize 后） |
| commission_paid_amount | DECIMAL(10,2) | DEFAULT 0 | 本单已收佣金累计 |
| repair_payment_proof | JSON | | 维修款支付凭证 URL 数组 |
...
| 表 | 说明 |
|----|------|
| merchant_commission_wallets | 每店一行：余额、冻结、扣款模式 auto/per_order |
| merchant_commission_ledger | 充值、扣佣、微信单笔支付、退款、差额调整流水 |
| merchant_commission_payment_intents | 微信支付预下单：recharge / order_commission |

**settings**：`commission_waive_insurance`（1=保险单免收门店佣金）、`commission_low_balance_threshold_yuan`（低余额提醒阈值）。

迁移脚本：`web/database/migration-20260321-merchant-commission.sql`。
